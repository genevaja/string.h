...
#define s21_NULL (void*)0
  ...
  { "Undefined error: 0", "Operation not permitted", "No such file or directory", "No such process",        \
    ...
    "No child processes",    "Resource deadlock avoided", "Cannot allocate memory",                         \
    "Permission denied", "Bad address", "Block device required", "Resource busy", "File exists",            \
    ...
    "Connection refused", "Too many levels of symbolic links", "File name too long",                        \
    "Host is down", "No route to host", "Directory not empty", "Too many processes",                        \
    ...
    "Program version wrong", "Bad procedure for program", "No locks available", "Function not implemented", \
    ...
    "Protocol error", "STREAM ioctl timeout", "Operation not supported on socket", "Policy not found",      \
...
int s21_char_to_string(char *str, va_list argptr, config *pointer);
int s21_string_to_string(char *str, va_list argptr, config *pointer);
